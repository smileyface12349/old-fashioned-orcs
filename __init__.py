from src.player import * #pygame has been imported in that file, so we don't need to import it again

pygame.init() #ensuring that everything we need will be initialised before starting

#Here we create the game window. The first argument is the native resolution the game runs at, and you can then add flags after that
#The ones I put here allow us to have a bigger window without needing to scale everything up, and also allows the user
#to resize the window as they like.
#Feel free to change the resolution if you feel that it's too small
